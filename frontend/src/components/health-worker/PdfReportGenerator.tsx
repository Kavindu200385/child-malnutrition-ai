...
function kv(pdf: PDF, y: number, label: string, val: unknown, right = false) {
  const x = right ? M + CW / 2 : M;
  font(pdf, 8.5, 'bold', [60, 60, 60]);
  pdf.text(label, x + 2, y);
  font(pdf, 8.5, 'normal', [30, 30, 30]);
  const safe = textValue(val);
  pdf.text(safe, x + 40, y);
}

function footer(pdf: PDF, pg: number, tot: number, name: unknown, id: unknown) {
  const y = PH - 7;
  hr(pdf, y - 3, [200, 200, 200]);
  font(pdf, 6.5, 'normal', [150, 150, 150]);
  pdf.text(
    `CMRAS Child Health Record  |  ${textValue(name, 'Child')}  |  ID: ${textValue(id)}  |  Generated: ${new Date().toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}`,
    M, y
  );
...
export async function generateProfessionalPdf(
  ...
  const childId = textValue(child.id);
  const childName = textValue(child.name, 'Child');
  ...
  stats.forEach((s, i) => {
    ...
    const v = pdf.splitTextToSize(textValue(s.val), sw - 4)[0];
  ...
  footer(pdf, pg, TOTAL, childName, childId);
  ...
  kv(pdf, y + 8, 'Full Name:', childName);
  kv(pdf, y + 8, 'Child ID:', childId, true);
  ...
  // Save
  const safe = childName.replace(/[^a-z0-9-_ ]/gi, '').trim().replace(/\s+/g, '_') || 'child';
  pdf.save(`CMRAS_Report_${safe}_${childId}_${new Date().toISOString().slice(0, 10)}.pdf`);
}
