...
import { Users, Building2, AlertTriangle, Activity, Stethoscope, UserCheck, MapPin } from 'lucide-react';
...
export function PdhsOverview() {
    ...
    pdhsAPI
      ...
      .then((res) => {
        ...
        if (res.data?.status === 'success' && res.data?.data) {
          ...
          setError(res.data?.message || 'Failed to load province dashboard');
        }
      })
      .catch((err) => {
        if (!cancelled) setError(err.response?.data?.message || 'Failed to load province dashboard');
      })
      .finally(() => {
  ...
  const districtPerformance = (data?.district_performance ?? []).map((d: any) => ({ ...d, district_name: (d.district_name || '').length > 18 ? (d.district_name || '').slice(0, 16) + '…' : d.district_name }));
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-900">Province Overview</h2>
        <p className="text-gray-600 mt-1">{provinceNames} – province-level dashboard (read-only)</p>
      </div>
