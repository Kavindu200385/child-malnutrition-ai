/**
 * PDHS Children – read-only list of children in province.
 * PDHS cannot register, add measurements, or modify children.
 */
...
export function PdhsChildrenView() {
  const PAGE_SIZE = 12;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <h2 className="text-2xl font-bold text-gray-900">Province Children (Read-only)</h2>
        <button
          type="button"
          onClick={load}
      ...
      <div className="bg-white rounded-lg shadow">
        ...
        {children.length === 0 ? (
          <div className="p-12 text-center text-gray-500">No children in your province.</div>
        ) : (
          <>
            <div className="grid grid-cols-1 items-stretch gap-6 p-6 md:grid-cols-2 lg:grid-cols-4">
              {paginatedChildren.map((child) => (
                <ChildProfileCard key={child.id} child={child} accent="indigo" />
              ))}
            </div>
            <PaginationControls
