/**
 * PDHS Health Workers – list province workers, activate/deactivate, view performance.
 */
import { useState, useEffect } from 'react';
import { Users, RefreshCw, UserCheck, UserX, BarChart2 } from 'lucide-react';
import { pdhsAPI } from '../../services/api';

export function PdhsHealthWorkersView() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <h2 className="text-2xl font-bold text-gray-900">Province Health Workers</h2>
        <button
          type="button"
          onClick={load}
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <table className="w-full">
          ...
          <tbody>
            {workers.length === 0 ? (
              <tr>
                <td colSpan={5} className="py-8 text-center text-gray-500">No workers in your province.</td>
              </tr>
            ) : (
              workers.map((w) => (
